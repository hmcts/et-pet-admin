module Admin
  class AcasCertificateSearchService
    ...
    def self.call(form)
      ...
      response = HTTParty.get("#{base_url}/certificates/#{form.number.gsub(/\s+/, '')}", format: :json,
                                                                                         headers: { 'EtUserId': form.current_admin_user.email, 'Accept' => 'application/json', 'Content-Type' => 'application/json' })
