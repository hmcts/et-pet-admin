class UploadedFile < ApplicationRecord
  ...
  scope :et3_user_files, -> { where(filename: ['et3_atos_export.pdf', 'additional_information.rtf']) }
  enum file_scope: { user: 'user', system: 'system' }, _suffix: true

  def self.ransackable_associations(auth_object = nil)
    ['claim_uploaded_files', 'file_attachment', 'file_blob', 'response_uploaded_files']
  end
